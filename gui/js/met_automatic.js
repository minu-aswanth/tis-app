...
fetch_users = function(){
			$('#auto_users').empty();
            $.ajax({
            	url: '../utils/get_met_auto_users.php',
            	success: function(result) {
...
$("#add_action_submit").click(function(){
    ...
    $.ajax({
		url: '../utils/add_met_auto_users.php',
...
function editRow(){
	...
	$.ajax({
		url: '../utils/edit_met_auto_users.php',
...
function deleteRow() {
	try{
		var name = $("input[type='radio'][name='mds']:checked").parent().parent().children()[2].innerHTML
		$.ajax({
				url: '../utils/del_met_auto_users.php',
				data :{user:name},
				type: 'POST',
				success: function(result) {
					if(result.length == 2){
