...
function update_plan(){
	try {
		...
		foreach ($signals as $signal) {
			...
			$j = 1;
			foreach ($signal->inter_stage_timings as $time) {
				$handle=$link->prepare("UPDATE `signal_timings` SET `InterStageTime`= :inter_stage_time WHERE (`SignalID` = :signal_id AND `StageNumber` = :stage_number) AND (`Plan_SCN` =:plan_scn)");
				$handle->bindParam(':inter_stage_time', $time);
				$handle->bindParam(':signal_id', $signal_id);
				$handle->bindParam(':stage_number', $j);
				$handle->bindParam(':plan_scn', $plan_scn);
				$handle->execute();
				$j++;
			}
