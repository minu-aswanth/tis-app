...
try {
	...
	$link = linkToTIS();
	$active = 1;
	$handle=$link->prepare("UPDATE `utmc_detector_static` SET `ShortDescription` = '$short_descrip', `Northing`='$northing', `Easting`='$easting', `IPAddress`='$ip', `Port`='$port', `Place`='$place', `LastUpdated`=now()  WHERE `utmc_detector_static`.`SystemCodeNumber` = '$scn'");
	$handle->execute();
	return "success";
}
