...
try {
	...
	$handle=$link->prepare("UPDATE `utmc_vms_static` SET `ShortDescription` = '$short_descrip', `Northing`='$northing', `Easting`='$easting', `IPAddress`='$ip', `Port`='$port', `Place`='$place', `LastUpdated`=now()  WHERE `utmc_vms_static`.`SystemCodeNumber` = '$scn'");
	$handle->execute();
